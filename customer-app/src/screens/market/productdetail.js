 ...
 * Premium Food & Drink Product Details View (Alternative Hook Target)
...
export default function ProductDetailScreen({ routeItem, onAddToBasket, onBackPress }) {
  ...
  const themePrimary = colors?.primary || DETAIL_COLORS.primary;

  const product = routeItem || DEFAULT_PRODUCT;
  const resolvedUnitPrice = Number(
    product?.basePrice ?? product?.price ?? DEFAULT_PRODUCT.basePrice
  );
  const unitPrice = Number.isFinite(resolvedUnitPrice)
    ? resolvedUnitPrice
    : DEFAULT_PRODUCT.basePrice;
      ...
      <View style={styles.imageHeaderFrame}>
        ...
        <View style={[styles.floatingBarRow, { paddingTop: insets.top + 10 }]}>
          <TouchableOpacity style={styles.circleIconBtn} onPress={onBackPress} activeOpacity={0.7}>
            <CustomIcon name="arrow-left" size={18} color={DETAIL_COLORS.textDark} />
          </TouchableOpacity>
          <TouchableOpacity style={styles.circleIconBtn} onPress={() => setIsFavorite(!isFavorite)} activeOpacity={0.7}>
            <CustomIcon name={isFavorite ? "favorite-filled" : "heart"} size={18} color={DETAIL_COLORS.primary} />
          </TouchableOpacity>
        </View>
      </View>
          ...
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollPaddingContent} bounces={false}>
            ...
            <View style={styles.titleRatingContainerRow}>
              ...
              <View style={styles.ratingBadgeElement}>
                <CustomIcon name="star" size={12} color={DETAIL_COLORS.primary} />
...
const styles = StyleSheet.create({
  masterContainer: { flex: 1, backgroundColor: DETAIL_COLORS.background },
  imageHeaderFrame: { width: width, height: height * 0.38, backgroundColor: DETAIL_COLORS.surfaceLight, position: 'relative' },
  ...
  detailsContentContainer: { flex: 1, backgroundColor: DETAIL_COLORS.background, borderTopLeftRadius: 28, borderTopRightRadius: 28, marginTop: -24, paddingHorizontal: 24, overflow: 'hidden' },
  ...
  productNameTitleText: { fontSize: 22, fontWeight: '900', color: DETAIL_COLORS.textDark, letterSpacing: -0.5, marginBottom: 4 },
  productPriceText: { fontSize: 14, fontWeight: '700', color: DETAIL_COLORS.textMuted },
  ...
  ratingValueLabelText: { fontSize: 12, fontWeight: '800', color: DETAIL_COLORS.primary, marginLeft: 4 },
  descriptionBodyText: { fontSize: 14, fontWeight: '600', color: DETAIL_COLORS.textMuted, lineHeight: 22, marginBottom: 24 },
  quantitySectionRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderTopWidth: 1, borderTopColor: DETAIL_COLORS.borderLine, paddingTop: 20 },
  quantityLabelText: { fontSize: 15, fontWeight: '800', color: DETAIL_COLORS.textDark },
  quantityControlGroup: { flexDirection: 'row', alignItems: 'center', backgroundColor: DETAIL_COLORS.surfaceLight, borderRadius: 20, padding: 4, borderWidth: 1, borderColor: DETAIL_COLORS.borderLine },
  qtyControlBtn: { width: 32, height: 32, borderRadius: 16, backgroundColor: '#FFFFFF', alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: DETAIL_COLORS.borderLine },
  qtyControlBtnText: { fontSize: 18, fontWeight: '700', color: DETAIL_COLORS.textDark, marginTop: -2 },
  qtyDisplayValue: { fontSize: 15, fontWeight: '700', color: DETAIL_COLORS.textDark, paddingHorizontal: 16 },
  basketActionFooter: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderTopWidth: 1, borderTopColor: DETAIL_COLORS.borderLine, paddingTop: 16, marginTop: 'auto', backgroundColor: '#FFFFFF' },
  basketTotalSummaryColumn: { flex: 0.4 },
  basketTotalLabelText: { fontSize: 11, fontWeight: '700', color: DETAIL_COLORS.textMuted, textTransform: 'uppercase', letterSpacing: 0.5 },
  basketTotalValueText: { fontSize: 20, fontWeight: '900', color: DETAIL_COLORS.textDark, marginTop: 2 },
